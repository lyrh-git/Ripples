  ...
  methods: {
    initChart: function() {
     /* let send={
      ...
      axios
        .post("http://localhost:8080/Ripples_API/Person/CloudServlet", this.$qs.stringify(send))
        .then(response => {
          this.cloudData = JSON.parse(JSON.stringify(response.data)).data;
        })
        .catch(error => {
          alert(error);
        });*/
      ...
      // maskImage.src="G:\\Node\\node_global_modules\\node_modules\\vue\\Ripples1\\static\\images_xingzuo\\baiyang.png"
        ...
        series: [
          {
            type: "wordCloud",
            gridSize: 20, //词间距
            sizeRange: [12, 30], //字的大小范围
            rotationRange: [-90,0], //词的旋转方向[-45,0,45,90]
            rotationStep:90,
            // markImage:markImage,
            textStyle: {
              //随机生成字体颜色
