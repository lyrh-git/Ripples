<template>
  <div>
    ...
    <div v-for="post in posts">
      ...
      :post_good_count="post.post_good_count" :post_comment_count="post.post_comment_count" :post_badges="post.post_badges" :post_collect_count="post.post_collect_count"
...
export default {
  ...
  methods: {
    initPosts: function() {
      ...
      axios
        .post(
          "/api/Ripples_API/Home/RealtimePostServlet",this.$qs.stringify(send)
        )
        .then(response => {
          this.posts= JSON.parse(JSON.stringify(response.data)).posts;
          // alert(this.posts[0].topic_id)
        })
        .catch(error => {
          alert(error);
        });
