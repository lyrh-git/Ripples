...
<div>
    <ul class="good-list nav">
        <li class="good-item" v-for="(item,index) in goods" :key="index" @click="togglePage(item)">
            <img class="good-item-image" :src="item.images[0]" />
            <h4>{{item.name}}</h4>
        </li>
    </ul>
</div>
...
<script>
import goods from "@/data/goods"; //引入goods的数据，@等价于src目录
